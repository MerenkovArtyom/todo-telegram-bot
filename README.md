...
- Планирование напоминаний по времени для существующих задач.
- Просмотр активных напоминаний командой `/reminders`.
- Удаление напоминаний командой `/unremind <номер>` или через inline-выбор номера.
...
- `/reminders` — показать активные напоминания и время до срабатывания.
- `/unremind <номер>` — удалить напоминание по номеру из списка `/reminders`.
...
## Напоминания (подробно)

### Как создать напоминание
1. Выполните `/remind` (или нажмите кнопку `/remind` на клавиатуре).
2. Бот покажет список текущих задач в виде inline-кнопок.
3. Выберите задачу.
4. Введите время в формате `ЧЧ:ММ` (например, `09:30`).
5. Бот сохранит напоминание.

Проверка времени:
- формат строго `^\d{2}:\d{2}$`;
- часы: `00..23`;
- минуты: `00..59`.

### Как рассчитывается время срабатывания
- Используется таймзона `Europe/Moscow` (MSK).
- Если введенное время еще не наступило сегодня — сработает сегодня.
- Если уже прошло — сработает завтра.

### Как отправляются напоминания
- При старте бота поднимается фоновый воркер.
- Воркер периодически проверяет напоминания в БД.
- Когда время наступило, бот отправляет сообщение:
  - `⏰ Напоминание: <текст задачи>`
- После успешной отправки напоминание удаляется из БД.

### Просмотр и удаление напоминаний
- `/reminders` показывает все активные напоминания с нумерацией:
  - номер напоминания;
  - текст задачи;
  - заданное время;
  - сколько осталось до срабатывания;
  - локальное время срабатывания (MSK).
- `/unremind <номер>` удаляет напоминание по номеру из списка.
- Если вызвать `/unremind` без номера, бот покажет inline-кнопки с номерами (аналогично `/done`).

### Автоочистка
- Если задача удалена через `/done`, связанные с ней напоминания удаляются.
- Если для напоминания задача уже не существует, такое напоминание удаляется воркером.
...
app/
  ...
  bot/
    ...
    handlers/
      ...
      reminders.py       # создание/просмотр/удаление напоминаний
  ...
  services/
    ...
    reminders.py         # бизнес‑логика напоминаний и воркер
  db/
    database.py          # подключение к SQLite и init схемы
    reminders_repo.py    # CRUD для напоминаний
    tasks_repo.py        # CRUD для задач
  schemas/
    reminder.py          # dataclass модели напоминаний
...
- `time_hhmm` — TEXT, время напоминания (`ЧЧ:ММ`);
- `next_fire_at` — TEXT, время срабатывания в ISO (UTC);
- `is_active` — INTEGER, флаг активности;
- `created_at` — TEXT, дата/время создания;
- `last_fired_at` — TEXT, дата/время последней отправки.
